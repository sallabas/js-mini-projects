...
app.get('/admin', (req, res) => {
    if (!req.session.adminId) {
        return res.redirect('/admin-login'); // Admin girişi yapılmadıysa login sayfasına yönlendir
    }
...
app.post('/admin-login', (req, res) => {
    ...
    db.get(sql, [name, email], (err, user) => {
        ...
        if (user.role !== 'admin') {
            // Role "admin" değilse hata mesajı
            return res.render('admin-login', { errorMessage: 'Access Denied: You are not an admin.' });
        }

        // Admin doğrulandı, oturumu başlatıyoruz
